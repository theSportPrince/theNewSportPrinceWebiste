  ...
  const fetchLiveMatch = async () => {
    try {
      const response = await axios.get(
        `https://rest.entitysport.com/v2/matches/?status=2&token=${process.env.REACT_APP_TOKEN}&paged=1&per_page=50`
      );
      setLiveMatchData(response.data.response.items);
      setLoading(false);
      setIsSuccess(true);
      setSuccessMessage("Completed Data fetch sucessfully")
    } catch (error) {
      setHasError(true);
      setErrorMessage(
          ...
          >
            <Box sx={{ display: "flex", width: "100%" }}>
              ...
              <Box sx={{ justifyContent: "flex-end" }}>
                <Button
                  sx={{
                    backgroundColor: "Green", // Change color based on match status
                    borderRadius: "10px",
                    marginTop: "10px",
                    marginRight: "10px",
                    alignItems: "center",
